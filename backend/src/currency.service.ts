...
async function getEuroRate(sourceCurrency: string): Promise<number> {
  ...
  if (!response.ok) throw new Error(`Could not convert ${sourceCurrency} to EUR`);
...
export async function convertToEuro(amount: number, sourceCurrency: string): Promise<number> {
